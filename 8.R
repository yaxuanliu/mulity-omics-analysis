...
clusterFile="risk.all.txt"      #分型结果文件
gmtFile="h.all.v7.5.1.symbols.gmt"                   #基因集文件
setwd("C:\\Users\\86188\\Desktop\\GBM-scRNA2\\21.GSVA")      #设置工作目录
...
gsvaResult=data.frame(gsvaResult,stringsAsFactors = F)
gsvaCluster=cbind(gsvaResult, Risk=cluster[,"risk"])
gsvaCluster[,1:ncol(gsvaCluster)-1]=as.numeric(as.factor(gsvaCluster[,1:ncol(gsvaCluster)-1]))
...
for(i in 1:ncol(comp)){
  #样品分组
  treat=gsvaCluster[gsvaCluster$Risk==comp[1,1],]
  con=gsvaCluster[gsvaCluster$Risk==comp[2,1],]
